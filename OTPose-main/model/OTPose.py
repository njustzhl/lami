...
from model.ad_conv import AdaptiveConv2d
...
class OTPose(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.offset_mask_combine_conv = CHAIN_RSB_BLOCKS(self.num_joints * 8, def_ch, offset_mask_combine_ch)
    ...
    # 使用DKD参数适配器提取特征
        prev_heatmaps_dkd = self.dkd_param_adapter(prev_heatmaps)
        next_heatmaps_dkd = self.dkd_param_adapter(next_heatmaps)
        pprev_heatmaps_dkd = self.dkd_param_adapter(pprev_heatmaps)
        nnext_heatmaps_dkd = self.dkd_param_adapter(nnext_heatmaps)

        # 自适应卷积
        p_b, p_c, p_h, p_w = current_heatmaps.shape
        pose_adaptive_conv = AdaptiveConv2d(p_b * p_c,
                                            p_b * p_c,
                                            7, padding=3,
                                            groups=p_b * p_c,
                                            bias=False)
        support_heatmaps_prev = pose_adaptive_conv(current_heatmaps, prev_heatmaps_dkd)
        support_heatmaps_next = pose_adaptive_conv(current_heatmaps, next_heatmaps_dkd)
        support_heatmaps_pprev = pose_adaptive_conv(current_heatmaps, pprev_heatmaps_dkd)
        support_heatmaps_nnext = pose_adaptive_conv(current_heatmaps, nnext_heatmaps_dkd)

        # 多帧特征融合
        alpha = 0.5
        support_heatmaps_prev_normalized = support_heatmaps_prev / support_heatmaps_prev.max()
        support_heatmaps_next_normalized = support_heatmaps_next / support_heatmaps_next.max()
        support_heatmaps_pprev_normalized = support_heatmaps_pprev / support_heatmaps_pprev.max()
        support_heatmaps_nnext_normalized = support_heatmaps_nnext / support_heatmaps_nnext.max()

        support_heatmaps_fuse_prev = alpha * support_heatmaps_prev_normalized + (1 - alpha) * support_heatmaps_pprev_normalized
        support_heatmaps_fuse_next = alpha * support_heatmaps_next_normalized + (1 - alpha) * support_heatmaps_nnext_normalized
        # 1
        ...
        diff_A = current_heatmaps - prev_heatmaps
        diff_B = current_heatmaps - next_heatmaps
        ...
        # # 融合支持热图
        # support_heatmaps_fuse = torch.cat([support_heatmaps_fuse_prev, support_heatmaps_fuse_next], dim=1)
        # # 残差连接：在特征融合前保存输入
        # input_to_offset_mask = torch.cat([branches, def_heatmaps, support_heatmaps_fuse, dif_heatmaps], dim=1)
        # trans_heatmaps = self.offset_mask_combine_conv(input_to_offset_mask)
        # # 残差连接：添加到输出
        # trans_heatmaps = trans_heatmaps + input_to_offset_mask
        # 融合支持热图
        support_heatmaps_fuse = torch.cat([support_heatmaps_fuse_prev, support_heatmaps_fuse_next], dim=1)
        support_heatmaps_fuse=support_heatmaps_fuse[:, ::2, :, :]
        support_heatmaps_fuse=F.interpolate(support_heatmaps_fuse, size=(96, 72), mode='bilinear', align_corners=False)
        trans_heatmaps = self.offset_mask_combine_conv(torch.cat([branches, def_heatmaps, support_heatmaps_fuse,dif_heatmaps], dim=1))
